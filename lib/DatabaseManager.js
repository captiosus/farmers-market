...
var API_URL = "https://www.zipcodeapi.com/rest/" + API_KEY;
...
DatabaseManager.prototype.getListingsByDistance = function(zipcode, distance, callback){
  var these = this;
  request(API_URL + '/radius.json/'+zipcode+'/'+distance+'/mile',function(error, response, body){
    assert.equal(null, error);
    var zipcodes = [];
    var parsed = JSON.parse(body)['zip_codes'];
    for (var zipcode in parsed){
        zipcodes.push(parsed[zipcode]['zip_code']);
    }
