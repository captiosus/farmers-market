 ...
 * @param {string} the username of the document to be updated
 * @param {dictionary} The provided fields.
*/
DatabaseManager.prototype.updateUser = function(username, fields, callback){
  this.mongoClient.connect(this.databaseUrl, function(error, database){
    ...
    database.collection(DatabaseManager.USERS).updateOne({
      ...
      if(error || !results){
        typeof callback == "function" && callback(false, "User does not exist.")
        return;
      }
      typeof callback == "function" && callback(results, null);
      return;
    });
  });
};
