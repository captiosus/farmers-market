...
/**
 * This is the name of the collection storing the listings.
 * @type {string}
 */
 ...
 * @param {string} username The username of the user who added the listing.
 * @param {string} title The title of the listing.
 * @param {string} description The description of the listing.
 * @param {Array<string>} pictures The names of the pictures associated with
 *   this listing.
 * @param {number} price The price of the listing.
 */
DatabaseManager.prototype.addListing = function(username, title, description,
                                                pictures, price) {
  this.mongoClient.connect(this.databaseUrl, function(error, database) {
    assert.equal(null, error);
    database.collection(DatabaseManager.USERS).insert({
 ...
 */
DatabaseManager.prototype.getListing(listingId, callback){
  this.mongoClient.connect(this.databaseUrl, function(error, database){
    assert.equal(null, error);
...
DatabaseManager.prototype.updateListing(listingId, fields, callback){
  this.mongoClient.connect(this.databaseUrl, function(error, database){
    assert.equal(null, error);
    database.collection(DatabaseMAnager.LISTINGS).updateOne({
      ...
      if(error){
        typeof callback == function && callback(error, 'Listing does not exist!');
        return;
      }
      typeof callback == function && callback(null, 'Listing updated!');
      return;
    })
  })
}
